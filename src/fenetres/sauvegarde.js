...
.quoi{font-size:.79rem;color:#8fa1b8;line-height:1.6;margin:0;max-width:62rem}
.quoi b{color:#e8edf5}
...
.stat .s{font-size:.72rem;color:#6f8098;margin-top:.2rem}
...
.mono{font-family:Consolas,monospace;font-size:.7rem;color:#6f8098}
...
.b.att{color:#f0d6a0;border-color:rgba(240,180,80,.35)}
...
input.t{width:100%;background:#0f1724;border:1px solid #2b3444;border-radius:8px;color:#e8edf5;font:inherit;padding:.5rem .65rem}
...
.garde.jaune{background:rgba(234,179,8,.12);border:1px solid rgba(234,179,8,.4);color:#f0d6a0}
.garde.rouge{background:rgba(239,68,68,.12);border:1px solid rgba(239,68,68,.4);color:#fca5a5}
.garde b{color:#e8edf5}
...
.msgsur{flex:1 1 auto;min-width:0;font-size:.79rem;color:#8fa1b8;
...
<div class="tete"><span class="ic">💾</span><h1>Sauvegarde &amp; Restauration</h1></div>
  ...
  function vueListe(){
    ...
    for (var i=0;i<l.length;i++){ var b=l[i];
      h += '<tr><td style="white-space:nowrap;font-weight:600">'+esc(b.quand)
        ...
        + '<td>'+b.total+' enreg.<div style="font-size:.72rem;color:#6f8098">'+b.produits+' produits · '+b.commandes+' cmd · '+b.factures+' fact.</div></td>'
        + '<td style="text-align:center">'+(b.r2Objects==null?'—':b.r2Objects)+'</td>'
        + '<td style="white-space:nowrap">'+esc(b.taille)+'</td>'
        + '<td style="color:#8fa1b8">'+esc(b.note||'—')+'</td>'
        + '<td class="acts">'
