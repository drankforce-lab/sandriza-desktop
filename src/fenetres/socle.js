...
const ICO = {
  ...
  /* ⚠ LA CORBEILLE PORTE UNE FLÈCHE DE RETOUR, pas seulement un bac (#113).
     Un bac tout seul dit << ça part >> ; ici c est exactement le contraire —
     l écran existe pour REVENIR EN ARRIÈRE. Le pictogramme doit dire la
     fonction, pas le geste qui a mené là. */
  corbeille: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18"/><path d="M8 6V4a1 1 0 011-1h6a1 1 0 011 1v2"/><path d="M5 6l1 14a2 2 0 002 2h8a2 2 0 002-2l1-14"/><path d="M14.5 12.5l-2.5 2.5 2.5 2.5"/><path d="M12 15h3a2 2 0 010 4"/></svg>',
...
/* == LA BANDE D EN-TETE DES TABLEAUX, EN MODE SOMBRE AUSSI ==================
   Signale le 2026-09-04, deux captures cote a cote : la ligne NUMERO / CLIENT /
   TOTAL / STATUT porte une bande beige en mode JOUR et RIEN en mode sombre.
   ⚠ CE N ETAIT PAS UNE REGRESSION, et il faut le savoir : la nuit n a JAMAIS eu
   cette bande. Les 43 fenetres qui declarent thead th n y posent qu une bordure
   basse et une couleur de texte ; seule la reprise html.jour ajoutait un fond.
   C est en rendant le mode jour correct que l ecart est devenu visible - et il a
   fallu trois versions de reparation avant de comprendre que je cherchais une
   regression la ou il y avait un MANQUE.
   ⚠ LA REGLE VIT DANS LE SOCLE, PAS DANS LES 43 FENETRES. Le socle est ecrit
   AVANT la feuille de chaque fenetre, mais aucune d elles ne pose de fond sur
   thead th : la declaration survit donc partout, et une seule ligne couvre tout.
   La reprise de jour, ecrite APRES, garde son beige.
   Aucun accent grave ici : le tout part dans un litteral de gabarit.
   ⚠ ET LE POIDS EST MESURE, pas choisi a l oeil. En jour, la bande #e9e5da sur
   la carte blanche donne un rapport de clarte de 1.26. Reprendre --f-bande
   (#1b2635) n aurait donne que 1.07 sur la carte #16202f : deux fois plus faible,
   donc une bande qu on ne verrait pas davantage. --f-entete vaut #26334a, soit
   1.29 - le meme poids visuel qu en jour.
   ⚠ ET C EST UN JETON A PART, pas --f-bande : celui-la sert deja a trois autres
   endroits dont la nuit ne doit pas bouger. */
thead th{background:var(--f-entete)}
