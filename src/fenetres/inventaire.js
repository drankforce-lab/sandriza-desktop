...
.badge.finale{background:#dc2626;color:var(--tx-sur-accent)}
...
${JS_ACTIVITE()}${JS_DIRE()}
  ...
  function dessinerListe(){
    ...
    var h = '<div class="carte">'
      + '<input aria-label="${T("Scannez une étiquette, ou tapez un nom de produit")}" id="rech" autocomplete="off" placeholder="${T("Scannez une étiquette, ou tapez un nom de produit…")}">'
    ...
    h += '<div class="carte plein"><h2>' + (enRecherche
        ? '${T("Résultats")} <span class="note">— ' + lignes.length + ' '
          + (lignes.length > 1 ? '${T("produits")}' : '${T("produit")}') + '</span>'
      ...
      if (enRecherche) {
        lignes.forEach(function(a){
          h += '<div class="lg" data-pid="' + esc(a.id) + '">'
            ...
            + '</span>'
            + '<span class="fin">' + a.unites + ' ' + (a.unites > 1 ? '${T("unités")}' : '${T("unité")}')
            + ' · ' + a.variantes + ' ' + (a.variantes > 1 ? '${T("variantes")}' : '${T("variante")}')
  ...
  function dessinerProduit(){
    ...
    h += '<h2>' + esc(PROD.nom)
      ...
      + ' <span class="note">· ' + VARS.length + ' '
      + (VARS.length > 1 ? '${T("variantes")}' : '${T("variante")}')
      + '</span></h2>';
    ...
    if (!lot.length) {
      ...
      +   ' aria-label="' + esc('${T("Quantité —")} ' + v.taille + ' / ' + v.couleur) + '"></td>'
          ...
          +   '</select></td>'
          + '<td class="c">' + (v.sku
              ? '<button class="mini" data-etiq="' + i + '" title="${T("Imprimer les étiquettes de cette variante")}"><span class="ic">🖨</span></button>'
              : '') + '</td>'
          + '</tr>';
      });
      h += '</tbody></table></div>';
      ...
      h += '<div class="pagi">'
        ...
        + (pages > 1
            ...
            : lot.length + ' ' + (lot.length > 1 ? '${T("variantes")}' : '${T("variante")}'))
        + '</span></div>';
    }
  ...
  function dessinerProduits(){
    ...
    h += '<div class="toolbar">'
      ...
      + (d.peutEcrire && !LOT ? '<button class="mini" id="btn-lot" title="${T("Appliquer ou retirer la vente finale sur plusieurs produits à la fois")}">${T("Vente finale en lot")}</button>' : '')
      + '</span></div>';
    ...
    corps.onchange = function(ev){
      ...
      if (id) {
        ...
        if (lot) lot.textContent = nc + ' '
          + (nc > 1 ? '${T("produits sélectionnés")}' : '${T("produit sélectionné")}');
      }
    };
    corps.onclick = function(ev){
      var t = ev.target;
  ...
  function apresSucces(r){
    ...
    if (c && c.manuel) {
      ...
      h += '<p class="" style="color:var(--tx-att)"><span class="ic">⚠</span> ' + c.adresses.length + ' client'
        ...
        + '<textarea rows="2" readonly aria-label="${T("Adresses des demandes en attente")}">' + esc(c.adresses.join(', ')) + '</textarea>';
    } else if (c) {
      h += '<p style="color:var(--tx-ok)"><span class="ic">✉</span> ' + c.envoyes + ' '
        + (c.envoyes > 1
            ? '${T("courriels de retour en inventaire envoyés")}'
            : '${T("courriel de retour en inventaire envoyé")}')
        + (c.echecs ? ', <span style="color:var(--tx-err)">' + c.echecs + ' '
            + (c.echecs > 1 ? '${T("échecs")}' : '${T("échec")}') + '</span>' : '') + '.</p>';
    }
    ...
    if (etiq.length) {
      h += '<p>${T("Imprimer les étiquettes code-barres selon les quantités saisies ?")} '
        + '<strong>' + total + '</strong> ' + (total > 1 ? '${T("étiquettes")}' : '${T("étiquette")}')
        + '${T(" pour ")}' + etiq.length + ' '
        + (etiq.length > 1 ? '${T("variantes")}' : '${T("variante")}') + '.</p>';
    }
    h += '<div class="fin2">'
  ...
  function toutesLesEtiquettes(){
    ...
    voile('<h3><span class="ic">🏷</span> ${T("Imprimer les étiquettes")}</h3>'
      + '<p>${T("Imprimer")} <strong>' + total + '</strong> ' + (total > 1 ? '${T("étiquettes")}' : '${T("étiquette")}')
      + '${T(" pour ")}' + items.length + ' '
      + (items.length > 1 ? '${T("variantes")}' : '${T("variante")}') + ' ${T("en stock ?")}</p>'
