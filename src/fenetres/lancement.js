...
.etat .ds{font-size:.82rem;color:#8fa1b8}
...
.src.ok{color:#8fa1b8}.src.warn{color:#facc15}.src.err{color:#f87171}
...
button.bsc{font:inherit;border:none;border-radius:8px;padding:.65rem 1.25rem;font-size:.88rem;font-weight:700;cursor:pointer;color:#fff;white-space:nowrap}
...
<div class="tete"><span class="ic">🚀</span><h1>Mode lancement</h1></div>
...
${JS_ACTIVITE}${JS_DIRE}
  ...
  function dessiner(){
    ...
    else if (D.envPresente) src = '<div class="src ok">🔗 État piloté par la variable Render <code>ELG_LAUNCHED=' + esc(D.envValeur) + '</code> — il survit aux déploiements.</div>';
