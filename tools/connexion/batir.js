...
p("      \"Ou scannez le code ci-dessous.\": \"Or scan the code below.\",");
...
p("      \"La fenêtre principale ne répond pas.\": \"The main window is not answering.\",");
...
p("        message: T('La fenêtre principale ne répond pas.') });");
p("    }");
...
p("    };");
p("    /* ⚠⚠ LE CODE QR NE SE CHARGE PLUS, IL EST DÉJÀ DESSINÉ (2026-09-11).");
p("       Il venait d un service externe, avec le secret TOTP en clair dans");
p("       l adresse ; il est maintenant encodé sur le poste et arrive par le pont.");
p("       Il n y a donc plus d image à charger, donc plus d échec de chargement à");
p("       guetter — << onerror >> n avait de sens que pour une image distante.");
p("       ⚠ CE QUI REMPLACE LE REPLI : si la case est vide (encodeur absent), on");
p("       le DIT, au lieu de laisser un carré blanc que personne ne sait lire. La");
p("       clé à recopier reste juste au-dessus, et c est elle la voie sûre. */");
p("    var wq = el('wz-qr');");
p("    if (wq && !wq.firstChild) {");
...
p("      apresLogin();");
p("    }");
p("    /* ══⚠⚠ LA BARRE DE MENUS SE REFAIT, ET C EST LE DÉFAUT DE LA 5.28.0.");
p("       Ses mots : << le menu est en anglais, quand je change la langue la");
p("       langue du menu ne change pas, et le menu n est plus cliquable, rien");
p("       ne s ouvre >>. UN SEUL DÉFAUT, DEUX SYMPTÔMES.");
p("       La barre est posée sur le CORPS, pas dans #cx-corps : << socle() >> ne la");
p("       touche donc pas, et << BARRE_FAITE >> l empêchait d être rebâtie. Elle");
p("       gardait ses intitulés d origine.");
p("       ⚠ ET C EST CE QUI CASSAIT LE CLIC : la coquille retrouve le menu");
p("       demandé PAR SON INTITULÉ. Panneau reconstruit en anglais, barre restée");
p("       en français — plus aucune correspondance, donc plus rien qui s ouvre.");
p("       ⚠ LA LEÇON : deux copies d une même donnée dans deux endroits qui ne");
p("       se rafraîchissent pas ensemble finissent par diverger, et la panne ne");
p("       ressemble JAMAIS à sa cause — ici, une traduction incomplète s est");
p("       manifestée par un menu mort. */");
p("    BARRE_FAITE = false;");
p("    barreEteindre();");
p("    var vieille = document.querySelector('.cx-barre');");
p("    if (vieille && vieille.parentNode) vieille.parentNode.removeChild(vieille);");
p("    barrePoser();");
